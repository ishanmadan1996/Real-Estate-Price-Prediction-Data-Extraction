...
urls_page = ['https://www.99acres.com/property-in-borivali-west-mumbai-andheri-dahisar-ffid-page-'+
             str(i) for i in range(1, 31)] #pagenation links

with open('C:\Users\Ishant\Desktop\\' + 'Borivali(West).csv', 'a') as f:
    writer = csv.writer(f)
        ...
        for span in ab: #go through all links found on the page
            try:
                ...
                for i in links:
                    ...
                    with open('C:\Users\Ishant\Desktop\\'+'Borivali(West).csv','a') as f:
                        writer = csv.writer(f)
                        rows = zip([superbuiltuparea], [area],[carpetarea],[configuration], [price],[price_per_sq_feet],[age],[floorno],[address],[parking],[furnishing] ,['Borivali West'],'\n')
                        for row in rows:
                            print row
                            writer.writerow(row)
                            f.close
            except Exception as e:
                with open('C:\Users\Ishant\Desktop\\' + 'log_Borivali(West).csv', 'a') as f1:
                         f1.write(str(url_extension+i['href'])+'\n')
                         f1.close()
    except Exception as e1:
        with open('C:\Users\Ishant\Desktop\\' + 'log_Borivali(West)_pagelinks.csv', 'a') as f1:
            f1.write(str(url_extension + i['href']) + '\n')
            f1.close()
